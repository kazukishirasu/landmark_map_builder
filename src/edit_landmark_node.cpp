#include "landmark_map_builder/edit_landmark_node.h"

namespace landmark_map_builder {
edit_landmark::edit_landmark() : pnh_("~"), server_(new interactive_markers::InteractiveMarkerServer("visualize_landmark"))
{
    ROS_INFO("Start edit_landmark_node");
    pnh_.param("debug", debug_, false);
    load_yaml();
    save_srv_ = nh_.advertiseService("/save_landmark", &edit_landmark::cb_save_srv, this);
}
...
void edit_landmark::cb_feedback(const visualization_msgs::InteractiveMarkerFeedbackConstPtr& feedback)
{
...
void edit_landmark::cb_add(const visualization_msgs::InteractiveMarkerFeedbackConstPtr& feedback)
{
    ...
    server_->insert(int_marker, boost::bind(&edit_landmark::cb_feedback, this, _1));
    menu_handler_.apply(*server_, marker_name);
    server_->applyChanges();
    ROS_INFO("add %s", marker_name.c_str());
}

void edit_landmark::cb_delete(const visualization_msgs::InteractiveMarkerFeedbackConstPtr& feedback)
{
...
bool edit_landmark::cb_save_srv(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
{
    if (save_yaml()){
...
void edit_landmark::initMenu()
{
    menu_handler_.insert("add", boost::bind(&edit_landmark::cb_add, this, _1));
    menu_handler_.insert("delete", boost::bind(&edit_landmark::cb_delete, this, _1));
}

void edit_landmark::main()
{
    initMenu();
    for (const auto& lm:landmark_list_){
        server_->insert(lm, boost::bind(&edit_landmark::cb_feedback, this, _1));
...
visualization_msgs::InteractiveMarker edit_landmark::makeInteractiveMarker(const std::string& name, const geometry_msgs::Point& position, size_t id)
{
...
visualization_msgs::InteractiveMarkerControl edit_landmark::makeMarkerControl(const std::string& name, size_t id)
{
...
visualization_msgs::InteractiveMarkerControl edit_landmark::makeMenuControl()
{
    visualization_msgs::InteractiveMarkerControl control;
    control.always_visible = true;
...
{
    ros::init(argc, argv, "edit_landmark_node");
    landmark_map_builder::edit_landmark el;
    el.main();
    return 0;
}